...
function NT_mtd(t, beta_init, c_init, paramNT)
    eps_NT = paramNT[1];
    paramLS = paramNT[2];
    paramf = paramLS[3];
    ...
    while(true)
        ...
        #gradb1, gradc1 = fgrad2(t, beta, c, paramf);
        ...
        delta_beta, delta_c, iter = CG2(t, beta, c, gradb, gradc, paramf);
        end
        #delta_beta1, delta_c1, iter1 = CG2(t, beta, c, gradb, gradc, paramf);
        #println(norm(delta_beta1-delta_beta))
        #println(norm(delta_c1-delta_c))
        t_path = [t_path; t];
        iter_path = [iter_path; iter]
        #println("norm distance", norm(delta_beta-delta_beta_1), "norm distance", norm(delta_c-delta_c_1))
        #end
    ...
    return beta, c, t_path, iter_path, timevec, count
end
