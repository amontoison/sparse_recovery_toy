...
function cgADMM(M_perptz, missing_indices, DFTdim, DFTsize, lambda, rho, maxt = 10000, tol = 1e-3)
    ...
    t = 0;
    err = 1;
    subgrad_vec = subgrad(missing_indices, z0, M_perptz, DFTdim, DFTsize);
    ...
    while((t<maxt) & (err>tol))
        ...
        subgrad_vec = [subgrad_vec; subgrad(missing_indices, z0, M_perptz, DFTdim, DFTsize)];
    end
...
function subgrad(missing_indices, z0, M_perptz, DFTdim, DFTsize)
    n = prod(DFTsize);
