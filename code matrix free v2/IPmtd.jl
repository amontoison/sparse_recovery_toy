...
function barrier_mtd(beta_init, c_init, t_init, paramB)
    ...
    while(true)
        ...
        # Newton step
        beta, c, t_path, iter_path, timevectemp, counttemp = NT_mtd(t, beta, c, paramNT)
        t_vec = [t_vec; t_path];
        iter_vec = [iter_vec; iter_path];
        timevec = [timevec; timevectemp];
        count_nt = count_nt+counttemp;
    ...
    return beta, c, t_vec, iter_vec, timevec, count_nt
end
